...
This module enforces critical separation of concerns in thermal management systems:
- Environment processes read sensors and may update sensor readings from hardware
- Controller processes read sensors but can only modify actuators (decision-making)
- No process should corrupt sensor readings from other processes

The permission system prevents controllers from accidentally or maliciously modifying
sensor readings, which would corrupt the thermal control feedback loop. This is
essential for safety in thermal management where incorrect sensor readings could
lead to inadequate cooling and hardware damage.

Permission checking occurs at runtime using call stack inspection to identify the
modifying process when State.with_device() is called. The permission matrix uses
class-based rules with inheritance support and ordered precedence checking.

Without this system, a buggy PID controller could modify temperature sensor readings
and create false feedback, potentially causing thermal runaway or inadequate cooling.
"""
